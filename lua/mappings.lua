...
vim.api.nvim_set_keymap('n', '<leader>gh', '<cmd>lua require("hubspot-completion").hover_translation()<CR>', { noremap = true, silent = true })
...
local function toggle_term_cmd(term_details)
  ...
  if type(term_details) == 'string' then term_details = {
    cmd = term_details,
    hidden = true,
  } end
...
vim.keymap.set('n', '<leader>b', function() utils.toggle_buffer_numbers() end, { noremap = true, silent = true })
for i = 1, 9 do
