...
function M.get_text_under_cursor()
	return vim.treesitter.get_node_text(vim.treesitter.get_node({ bufnr = 0 }), 0)
end
...
function M.find_root_git_dir()
	return M.buffer_find_file_dir(M.buffer(), ".git")
end
...
function M.buffer_find_file(bufnr, filename)
	...
	if vim.fn.filereadable(buffer_path) == 0 then
		return nil
	end
...
function M.buffer_find_file_dir(bufnr, filename)
	M.buffer_find_file(bufnr, filename .. "/..")
end
